...
/** Map the report content evidence sections into EvidenceTrail steps. */
function evidenceStepsFromContent(content) {
  const c = content || {};
  const provenance = c.provenance || {};
  const calculation = c.calculation || {};
  const lineage = c.lineage || {};
  const validation = c.validation || {};
  const fmt = (obj) => (obj && typeof obj === 'object' ? JSON.stringify(obj) : String(obj || ''));

  const steps = [];
  if (lineage && Object.keys(lineage).length) {
    steps.push({ id: 'source', label: 'Source lineage', detail: fmt(lineage), tone: 'complete' });
  }
  if (provenance && Object.keys(provenance).length) {
    steps.push({ id: 'factor', label: 'Factor provenance', detail: fmt(provenance), tone: 'complete' });
  }
  if (calculation && Object.keys(calculation).length) {
    steps.push({ id: 'calculation', label: 'Calculation information', detail: fmt(calculation), tone: 'complete' });
  }
  if (validation && Object.keys(validation).length) {
    steps.push({ id: 'validation', label: 'Validation', detail: fmt(validation), tone: 'complete' });
  }
  return steps;
}
        ...
        <div className="v3-detail-section">
          ...
          {ready && content ? (
            <>
              ...
              <EvidenceTrail steps={evidenceStepsFromContent(content)} title="Evidence trail" />
            </>
          ) : (
