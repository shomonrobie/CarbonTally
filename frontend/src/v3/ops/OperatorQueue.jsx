...
export default function OperatorQueue() {
  ...
  const openItem = async (itemId) => {
    ...
    setItem(null);
    setNotice('');
    // The workspace payload needs the item; load through the ops workspace API
    // by using getOpsBatchItems to refresh the current item from the list.
    const list = items.length ? items : await loadBatchItems(activeBatchId);
  ...
  const afterChange = async (itemId) => {
    const list = await loadBatchItems(activeBatchId);
    const found = list.find((i) => i.id === itemId);
    if (found) setItem(found);
  };
  ...
  return (
    <div>
      ...
      {item ? (
        <ExtractionPanel
          item={item}
          batch={items.find((i) => i.id === item.id)}
          items={items}
          api={INTERNAL_API}
