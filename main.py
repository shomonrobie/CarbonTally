...
def process_fuel_data(df: pd.DataFrame) -> tuple:
    ...
    date_col = next((c for c in df.columns if 'date' in c.lower()), 'CarbonTally')
    ...
    df = df.rename(columns={date_col: 'CarbonTally', vol_col: 'Volume (L)', reg_col: 'Vehicle Registration', fuel_col: 'Fuel Type'})
    
    # 2. Clean Data
    df['CarbonTally'] = pd.to_datetime(df['CarbonTally'], format='mixed', dayfirst=True, errors='coerce').dt.strftime('%Y-%m-%d')
    ...
    clean_cols = ['CarbonTally', 'Vehicle Registration', 'Standardized Fuel', 'Volume (L)', 'DEFRA Factor (kgCO2e/L)', 'Total kgCO2e', 'needs_review', 'review_reason']
