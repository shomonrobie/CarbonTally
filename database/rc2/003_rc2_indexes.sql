...
--   1. RECONCILES that family with CONCURRENTLY + IF NOT EXISTS so the suite
--      self-heals on any scheme that started without a given index (and re-runs
--      cleanly); and
...
--
-- WHY CONCURRENTLY (and non-transactional): CREATE INDEX CONCURRENTLY cannot
-- run inside a transaction block, so this file is deliberately NOT wrapped in
-- BEGIN/COMMIT. Each statement is self-contained; a failure is non-destructive
-- (only the offending index is skipped) and can be re-run.
...
-- ============================================================================
-- All IF NOT EXISTS: absent on this scheme → created; present → no-op. Uses
-- CREATE INDEX CONCURRENTLY so records are not locked during a re-run.
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS customer_documents_document_type_id_idx
...
-- ---------------------------------------------------------------------------
CREATE INDEX CONCURRENTLY IF NOT EXISTS dpq_emission_factor_used_idx
