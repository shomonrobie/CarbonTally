...
async def get_system_settings(supabase_client):
    ...
    try:
        ...
        if result.data:
            ...
            # If settings_json exists, use it as source of truth
...
async def validate_file_upload(file: UploadFile, settings: dict, is_batch: bool = False):
    ...
    # For batch uploads, check total size
    if is_batch:
        # This will be checked separately in the batch endpoint
        pass
...
def get_emission_factor(supabase_client, activity_type: str, reporting_year: int = None):
    ...
    Now handles activity type mapping.
    """
    try:
...
def process_fuel_data(df: pd.DataFrame, supabase_client) -> tuple:
    ...
    def normalize_fuel(fuel):
        ...
        if 'diesel' in fuel_str: return 'Diesel'  # Will map to 'Diesel (DERV)'
        if 'petrol' in fuel_str or 'gasoline' in fuel_str: return 'Petrol'  # Will map to 'Petrol (Unleaded)'
    ...
    # ✅ Get factors from database with proper mapping
    factors = []
...
def process_utility_data(df: pd.DataFrame, supabase_client) -> tuple:
    ...
    def normalize_utility_type(utype):
        ...
        if 'electric' in utype_str: return 'Electricity'  # Will map to 'UK Electricity Grid'
...
# ==========================================
# ADD DEFRA FACTOR MAPPING ENDPOINT
# ==========================================

@app.get("/api/defra-mapping")
async def get_defra_mapping():
    ...
    return {
        ...
        "available_factors": await get_available_defra_factors()
    }


@app.get("/api/defra-factors/{reporting_year}")
async def get_defra_factors_by_year(reporting_year: int):
    """
...
async def get_available_defra_factors():
    """
    Helper to get all available DEFRA factors grouped by year.
    """
    try:
        if supabase is None:
            return {}
        ...
        result = supabase.from_('defra_conversion_factors') \
            .select('reporting_year, activity_type, co2e_multiplier') \
            .order('reporting_year', desc=True) \
            .execute()
        
        factors_by_year = {}
        for factor in (result.data or []):
            year = factor['reporting_year']
            if year not in factors_by_year:
                factors_by_year[year] = []
            factors_by_year[year].append({
        ...
        print(f"⚠️ Error getting available factors: {e}")
        return {}
# ==========================================
...
async def upload_csv(
    ...
    try:
        ...
        # Read the file (use file_bytes from validation to avoid re-reading)
        if not file_bytes:
            file_bytes = await file.read()
...
async def upload_batch(
    ...
    try:
        ...
        # ⬇️ UNCOMMENT BELOW FOR PREMIUM USERS ⬇️
        """
        # Get system settings
        settings = await get_system_settings(supabase)
        
        # Check if bulk upload is enabled (premium feature)
        max_batch_files = settings.get('max_batch_files', 0)
        max_total_batch_size_mb = settings.get('max_total_batch_size_mb', 200)
        
        # Check if bulk upload is available (0 means disabled)
        ...
        if len(files) > max_batch_files:
            raise HTTPException(
                status_code=400,
                detail=f"Too many files. Max {max_batch_files} files allowed per batch."
            )
        ...
        if total_size_mb > max_total_batch_size_mb:
            raise HTTPException(
                status_code=400,
                detail=f"Total batch size too large. Max {max_total_batch_size_mb}MB. Current: {total_size_mb:.1f}MB"
            )
        ...
        # Validate and clean organization_id
        import uuid
        valid_org_id = None
        ...
        # Get emission factors for validation
    ...
    try:
        ...
        result = supabase.from_('system_settings') \
            .upsert({
                ...
                'max_file_size_mb': settings_data.get('max_file_size_mb', 50),
                'allowed_file_types': settings_data.get('allowed_file_types', ['pdf', 'csv', 'xlsx', 'jpg', 'jpeg', 'png']),
                'enable_auto_repair': settings_data.get('enable_auto_repair', True),
                'max_batch_files': settings_data.get('max_batch_files', 20),
                'max_total_batch_size_mb': settings_data.get('max_total_batch_size_mb', 200),
                'data_retention_days': settings_data.get('data_retention_days', 365),
...
async def get_current_user(
    ...
    try:
        ...
        print(f"🔍 Attempting to authenticate user...")
        ...
        try:
            ...
            # Try to decode token manually as fallback
            try:
                import jwt
        ...
        try:
            ...
            print(f"📊 Staff query result: {staff_result.data if staff_result else 'None'}")
            
        except Exception as staff_error:
            print(f"❌ Staff query error: {staff_error}")
            # Continue as viewer if staff query fails
            staff_result = None
        
        # ✅ If no staff profile, return as viewer
        if not staff_result or not staff_result.data:
            print(f"ℹ️ User {user_email} not found in staff_profiles, assigning viewer role")
            return AuthUser(
                user_id=user_id,
                email=user_email,
                role="viewer",
        ...
        role = staff_data.get('role', 'viewer')
        permissions = ROLE_PERMISSIONS.get(role, ROLE_PERMISSIONS.get("viewer", {}))
...
@app.get("/admin/users")
async def get_users(current_user: AuthUser = Depends(require_role(["admin", "data_approver"]))):
    # ... endpoint logic

# Require specific permission
@app.post("/admin/extract")
async def extract_data(current_user: AuthUser = Depends(require_permission("can_extract"))):
    # ... endpoint logic

# Get current user info
@app.get("/api/me")
async def get_my_info(current_user: AuthUser = Depends(get_current_user)):
    return current_user
"""
...
async def test_roles(
    ...
    """Test endpoint to check role-based access."""
    return {
...
async def test_permissions(
    current_user: AuthUser = Depends(require_permission("can_view_users"))
):
    """Test endpoint to check permission-based access."""
    return {
