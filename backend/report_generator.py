...
class EnhancedSustainabilityReportPDF(FPDF):
    ...
    def add_paragraph(self, text: str):
        ...
        self.multi_cell(190, 6, sanitize_text(text))
        self.ln(2)
    ...
    def add_section_title(self, title: str):
        ...
        self.cell(0, 12, sanitize_text(title), 0, 1, 'L')
    ...
    def add_metric_box(self, label: str, value: str, unit: str = ""):
        """Add a metric box for key metrics display."""
        self.set_fill_color(*self.colors['accent'])
        self.set_draw_color(*self.colors['primary'])
        
        # Draw box
        self.rect(self.get_x(), self.get_y(), 45, 25, 'DF')
        
        # Label
        self.set_font('Helvetica', '', 8)
        self.set_text_color(*self.colors['text_light'])
        self.set_xy(self.get_x() + 3, self.get_y() + 3)
        self.cell(39, 5, sanitize_text(label), 0, 1, 'L')
        
        # Value
        self.set_font('Helvetica', 'B', 14)
        self.set_text_color(*self.colors['secondary'])
        self.set_x(self.get_x() + 3)
        self.cell(39, 10, sanitize_text(value), 0, 0, 'L')
        
        # Unit
        if unit:
            self.set_font('Helvetica', '', 7)
            self.set_text_color(*self.colors['text_light'])
            self.cell(0, 10, sanitize_text(unit), 0, 1, 'L')
        
        self.set_x(self.get_x() + 50)  # Move to next box position
...
class EnhancedSustainabilityReportGenerator:
    ...
    def _fetch_organization_data(self):
        ...
        if self.org_data is None:
            ...
            self.org_data = org_res.data[0] if org_res.data else {}
            self.organization_name = sanitize_text(self.org_data.get('name', 'Unknown Organization'))
            self.company_number = sanitize_text(self.org_data.get('company_number', 'N/A'))
            ...
            try:
                ...
                if metadata_res.data:
                    self.org_metadata = metadata_res.data[0]
    ...
    def _calculate_yoy_comparison(self) -> Optional[YearOverYearComparison]:
        """Calculate year-over-year comparison."""
        if not self.previous_year_data:
            return None
    ...
    def _generate_enhanced_pdf(self, pdf):
        ...
        scope_data = [
            ['Scope 1', f"{self.scope_totals.get('Scope 1', 0)/1000:,.2f}", f"{(self.scope_totals.get('Scope 1', 0)/1000/self.total_emissions_tonnes*100) if self.total_emissions_tonnes > 0 else 0:.1f}%"],
            ['Scope 2', f"{self.scope_totals.get('Scope 2', 0)/1000:,.2f}", f"{(self.scope_totals.get('Scope 2', 0)/1000/self.total_emissions_tonnes*100) if self.total_emissions_tonnes > 0 else 0:.1f}%"],
            ['Scope 3', f"{self.scope_totals.get('Scope 3', 0)/1000:,.2f}", f"{(self.scope_totals.get('Scope 3', 0)/1000/self.total_emissions_tonnes*100) if self.total_emissions_tonnes > 0 else 0:.1f}%"],
        ]
