# CarbonTally Final Architecture Blueprint v1.3
...
A consultant is a professional organization or CarbonTally customer organization operating with Consultant product capabilities to manage client organizations.

Consultants access `/consultant`.

Consultant access is based on active client relationships/grants.

A Consultant may create and onboard new client organizations through the authorized Consultant customer-creation workflow.

An existing CarbonTally customer organization may acquire Consultant capabilities by upgrading its CarbonTally subscription/product entitlement. This is an additive capability upgrade: the existing organization identity, customer data, users, history, billing history, provenance and processing records are preserved. The organization is not converted into a different tenant and no data migration or duplication is required.

Consultant subscription/product entitlement and client-organization access are separate concerns. A Consultant subscription does not grant access to an existing CarbonTally organization; access to an existing organization requires an authorized active Consultant-client relationship/grant.
...
> **Clarification (10 September 2026):** `processing_origin` records the **processing
> capacity/control path** (CarbonTally internal vs Processing Entity) — *not* the
> identity of the individual human actor. The human actor who performed a stage is
> recorded separately in the actor/provenance columns and the append-only audit
> events. See the *Processing model — dimensional clarification* subsection below.
...
### Processing model — dimensional clarification (documentation amendment, 10 September 2026)

> **Status:** documentation clarification only. This amendment records the model that
> the architecture and implementation **already** express. It introduces no new
> capability, state, schema or policy, and it changes no workflow routing.

The following seven concepts are **distinct** and must not be conflated:

| Concept | Meaning |
|---|---|
| **Actor** | Who/which participant performs or participates in processing: **Organisation**, **Consultant**, **CarbonTally internal**, **Processing Entity** |
| **Processing mode** | **Automatic** or **manual** |
| **Input type** | PDF, CSV, XLS/XLSX, image, etc. |
| **Workflow state** | The item/job lifecycle/pipeline state |
| **Processing origin** | The manual-processing capacity/control path: **CarbonTally internal** vs **Processing Entity** |
| **Provenance** | Evidence of who performed an action, plus source/factor/machine provenance |
| **Entitlement** | Processing entitlement owned by the **client organisation** |

**Four processing actors.** CarbonTally is designed so that all four participate in
processing, where authorised: the Organisation (customer), the Consultant (acting for
an engaged client organisation), CarbonTally internal processing staff, and an external
Processing Entity. Automatic processing additionally executes through the platform's
system/automatic worker.

**Automatic and manual processing are an independent dimension.** The processing
**mode** is a separate axis from **who** performs the work; it is not an actor
category. The model therefore permits the following actor/mode combinations where the
actor is authorised for the work and, for manual actions, where the workflow state
allows the action:

| Actor | Automatic | Manual |
|---|---|---|
| Organisation | Yes (uploads/enqueues work for automatic processing) | Yes (extract / map / validate / calculate, and correction of unresolved automatic output) |
| Consultant | Yes (where authorised for the client organisation) | Yes (where authorised, via the consultant processing capabilities) |
| CarbonTally internal | Yes | Yes |
| Processing Entity | Not applicable — PEs are a **manual** processing capacity | Yes (assigned work) |

Consequently, the following must **not** be stated or implied: Consultant = manual;
Processing Entity = manual in the generic sense; Organisation = automatic;
CarbonTally internal = automatic. In particular, **an actor's identity never determines
the processing mode** — a Consultant, an Organisation or CarbonTally internal staff may
each perform automatic and/or manual processing where authorised.

**Processing Entities are additional controlled manual-processing capacity.** As stated
in §9 ("CarbonTally QC is an independent internal quality gate"), PEs are *additional
controlled processing capacity, not the exclusive source of manual processing*.
Manual processing is not defined by, nor confined to, Processing Entities.

**Input type is independent** of actor, processing mode and workflow state: PDF, CSV,
XLS/XLSX and image inputs are handled by the same canonical pipeline.

**Automatic → manual fallback is an existing architectural/workflow pattern**, not a
Phase-6 feature. Automatic extraction/mapping runs first; where the result is
unresolved, ambiguous or below the configured confidence/validation gates, the work
moves into human correction/manual processing (extraction, mapping, validation or
calculation) by an actor authorised for that work, and then continues through the
canonical pipeline. For CSV/Excel, automatic mapping may resolve the data directly, and
unresolved data can be corrected manually; for PDF, automatic extraction and mapping
may succeed, and insufficient extraction or mapping confidence routes the work into
human correction/manual processing.

**`processing_origin` is not actor identity and is not processing mode.** It identifies
the **manual-processing capacity/control path** — CarbonTally internal processing vs
Processing Entity processing — and affects which upstream controls apply (the internal
path has no PE Review / PE QC; the PE path does). It is stored once, immutably, on the
item and is preserved as auditable provenance so CarbonTally QC can verify processed
data regardless of origin (§11; CT-QC-002/004). The human actor who performed a stage
is recorded separately through the actor/provenance columns and the append-only audit
events. See `CARBONTALLY_V1.2_DUAL_ORIGIN_WORKFLOW_DESIGN.md` §4 for the authoritative
origin model, and `CARBONTALLY_P6_2_PO_DECISION_REGISTER.md` for the ratified decision
that **no consultant value is added to `processing_origin`**.
...
Billing remains provider-neutral and **server-authoritative**.
...
- subscription state;
- pricing rules;
- effective-dated/versioned commercial configuration;
- orders and payment status/references;
- auditability;
- client-capacity limits where applicable;
- feature entitlements such as Consultant and White-Label capabilities.

**Server-Side Commercial Authority is mandatory:** all prices, totals, discounts, taxes where applicable, plan metadata, client-capacity limits, feature entitlements, billing-cycle rules and checkout amounts must be resolved or independently validated by the CarbonTally server against authoritative database configuration. Client/frontend supplied prices, totals or entitlement claims must never be trusted.

The payment provider is payment infrastructure, not the authoritative CarbonTally commercial or entitlement ledger.

Payment-provider integration remains an adapter layer. Provider-specific identifiers and APIs must remain isolated from CarbonTally's provider-neutral commercial domain.
...
# 31.3 Consultant Commercial and Billing Amendment — 5 September 2026

The following Product Owner decisions are frozen for the Consultant commercial model:

### CT-CONSULT-001 — Consultant self-registration

Consultant self-registration is permitted. Registration creates the Consultant identity/profile and does not, by itself, authorize access to existing CarbonTally customer organizations.

### CT-CONSULT-002 — Consultant-created client organizations

A Consultant may create and onboard new client organizations through the authorized Consultant customer-creation workflow. The resulting Consultant-client relationship may be activated as defined by the existing approved workflow.

### CT-CONSULT-003 — Existing CarbonTally organizations

A Consultant may not claim or attach itself to an existing CarbonTally organization without explicit authorization from that organization.

An existing CarbonTally customer organization may itself acquire Consultant capabilities by upgrading its CarbonTally subscription/product entitlement. This upgrade is additive and preserves the organization's identity, data, users, history, billing history and provenance.

### CT-CONSULT-004 — Consultant/client relationship termination

When a Consultant-client service relationship ends, CarbonTally preserves the client organization and its data, provenance, history and processing records. The Consultant's access is revoked through the existing Consultant-client lifecycle. The former client may subsequently operate as a direct CarbonTally customer without recreating or migrating its organization.

### CT-BILL-001 — Server-authoritative commercial state

CarbonTally's database and server-side commercial services are authoritative for plans, prices, entitlements, client capacity, White-Label availability, billing cycles, discounts, orders and checkout amounts.

Frontend/client-supplied price, total, discount, plan metadata or entitlement claims must never be trusted.

### CT-BILL-002 — Provider-neutral billing

Payment providers remain infrastructure adapters. CarbonTally retains the authoritative application-level subscription, entitlement, order, credit and usage state.

### CT-BILL-003 — Consultant commercial configuration

Consultant plan definitions, pricing, active-client capacity, team limits, White-Label features and related commercial configuration must be administrable from the CarbonTally Admin control plane and must not be hard-coded into business logic.

Final public pricing remains a commercial decision and may be changed through authorized server-side configuration without requiring application-code changes.
...
- the final Consultant subscription/product catalogue, pricing and commercial packaging;
- the exact server-side commercial configuration/checkout implementation;
