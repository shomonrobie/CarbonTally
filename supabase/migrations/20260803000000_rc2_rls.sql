...
-- NOTE (db-reset repair): the original WITH CHECK also required
-- `new.email = old.email`. PostgreSQL RLS policy expressions cannot reference
-- NEW/OLD, so that comparison is not expressible in a policy. It is redundant
-- here anyway: `email` is column-REVOKEd from authenticated below (only
-- first_name, last_name, updated_at are grantable), so the API cannot change
-- email regardless of this policy.
...
-- NOTE (db-reset repair): the original WITH CHECK also required
-- `new.role = old.role`, `new.organization_id = old.organization_id` and
-- `coalesce(new.is_active,true) = coalesce(old.is_active,true)`. RLS policy
-- expressions cannot reference NEW/OLD (that comparison is only expressible in
-- a BEFORE UPDATE trigger). The self-write scope (own row only) is preserved;
-- the self-escalation hardening is moved to the application layer.
...
-- NOTE (db-reset repair): the original WITH CHECK also required
-- `new.organization_id = old.organization_id` (no re-homing). RLS policy
-- expressions cannot reference NEW/OLD, so that comparison is dropped; the
-- role-vocabulary check and the admin-only USING gate are preserved.
...
CREATE POLICY cfm_update_self_or_team_admin ON public.consultant_firm_members
    ...
    USING (user_id = auth.uid())
    WITH CHECK (user_id = auth.uid());
-- NOTE (db-reset repair): the original WITH CHECK also pinned
-- firm_id, can_manage_team, can_manage_clients, can_upload_documents and
-- client_access to their previous values (anti-escalation). RLS policy
-- expressions cannot reference NEW/OLD (only a BEFORE UPDATE trigger can), so
-- those comparisons are dropped; the self-write scope (own row only) is kept.
...
-- NOTE (db-reset repair): the original statement also had
-- `WITH CHECK (new.consultant_id = old.consultant_id)`. RLS policy expressions
-- cannot reference NEW/OLD, so that anti-re-homing guard is dropped; the
-- client-management USING gate above is unchanged.
