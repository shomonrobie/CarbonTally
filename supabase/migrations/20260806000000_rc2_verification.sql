-- ============================================================================
-- CarbonTally v1.0 RC2 — Post-Migration Verification Suite (REPAIR RELEASE)
-- File 007 of 008 (verification; companion to 008 release notes).
-- Adapted from database/rc1/007_rc1_verification.sql to the ACTUAL RC2 state.
--
-- RC2 DELTAS baked in (vs RC1's expectations):
--   1. Section 3a/3c — `emissions_logs_unit_fkey` was DROPPED (RC2-H2) and the
--      unit column is deliberately NOT FK'd. Expected FKs: 10, not 11; and the
--      emissions_logs.unit orphan check is REMOVED (unit is free text, per H2).
--   2. Section 4b — the two COALESCE/widened unique indexes rebuilt in 002
--      (`usage_tracking_org_month_uniq`, `emission_factors_year_activity_country_uniq`)
--      are verified present; their new definitions are shown for inspection.
--   3. Section 4a/10 — 003 added three indexes beyond RC1's 18:
--        password_reset_tokens_user_id_idx        (L3)
--        dpq_emission_factor_used_idx             (FK-support)
--        customer_documents_document_type_id_idx  (FK-support)
--      Expected 003 indexes: 21, not 18.
--   4. Section 5a/5g — K4 names reflect the ACTUAL baseline names
--      (…_status_check, organization_members_role_check); customer_subscriptions
--      status was WIDENED in 002 to the 9-value Stripe list.
--   5. Section 6 — RC2 004 generatively secures every organization_id table
--      (not a fixed "36"): verification is a structural GAP check, not a magic
--      count. Security helpers total 4 (is_org_member, is_org_active,
--      is_org_admin_or_owner, is_org_consultant).
--   6. Section 7d — functions total 6 (set_updated_at, anonymise_user + 4 RLS
--      helpers).
--
-- READ-ONLY: every statement is a SELECT or a DO block that only RAISEs
-- NOTICE. Nothing here writes. Safe to run against production as any role with
-- catalogue read access (run as the migration owner / postgres role on Supabase
-- for full visibility of pg_policies and pg_index).
--
-- Run order: after all of 001–006 have been applied, BEFORE Gate 4 (RLS
-- penetration matrix) and BEFORE application smoke tests. A non-empty FAIL set
-- (where a pass criterion is stated) must be resolved before sign-off.
-- ============================================================================
...
-- 1a. Old names must NOT exist (table, column, or dropped legacy frame).
-- PASS = empty set. (Covers RC2 001 R1/R2/R3: each legacy defra column dropped
-- or renamed is proven absent here.)
...
-- ============================================================================
-- SECTION 3 — FOREIGN KEYS (002 F1 + RC2-H2)
-- ============================================================================

-- 3a. FK inventory with validation status. PASS = 10 rows, all validated = true.
--     `emissions_logs_unit_fkey` must NOT appear (it was dropped by RC2-H2).
...
-- 3a-loss-check: the dropped H2 FK must be gone. PASS = empty.
SELECT con.conname, cl.relname AS child_table
 FROM pg_constraint con
 JOIN pg_class cl ON cl.oid = con.conrelid
 JOIN pg_namespace n ON n.oid = cl.relnamespace
WHERE n.nspname = 'public' AND con.conname = 'emissions_logs_unit_fkey';
...
-- (No emissions_logs.unit check: unit is deliberately NOT FK'd after RC2-H2.)
...
-- PASS = all six counts 0 (the SET NOT NULL guard in 002 enforced this).
...
-- ============================================================================
-- SECTION 4 — INDEXES (003 I1–I5 + FK-supporting + L3; 002 K5 unique-backed)
-- ============================================================================

-- 4a. Presence of all 21 indexes from 003. PASS = 21 rows.
SELECT i.indexname, i.tablename
 ...
 WHERE i.schemaname = 'public'
  AND i.indexname IN (
      ...
      'customer_documents_supplier_id_idx','customer_documents_document_type_id_idx',
      'dpq_customer_document_id_idx','dpq_emission_factor_used_idx',
      -- L3
      'password_reset_tokens_user_id_idx')
 ORDER BY i.indexname;

-- 4b. K5 unique-backed indexes from 002. PASS = 7 rows. (The two RC2-rebuilt
-- keys are shown with indexdef to confirm the widened/COALESCE shape.)
...
-- 4d. Duplicate/overlapping index scan. Review any hits manually. Pre-existing
-- Supabase indexes here are a Gate 1 finding, not an RC2 failure.
...
-- ============================================================================
-- SECTION 5 — CHECK CONSTRAINTS (001/002: C1 presence, K1–K4 + H4)
-- ============================================================================

-- 5a. RC2 CHECK inventory with validation status (baseline names).
-- PASS = rows, all validated = true. (Includes the WIDENED
-- customer_subscriptions_status_check replaced by 002.)
SELECT con.conname, cl.relname AS table_name, con.convalidated AS validated,
 ...
 WHERE n.nspname = 'public' AND con.contype = 'c'
   AND ( con.conname = 'facilities_postcode_or_eircode_check'
      ...
      OR con.conname IN ('processing_queue_status_check',
                         'document_processing_queue_status_check',
                         'customer_documents_status_check',
                         'organization_members_role_check',
                         'customer_subscriptions_status_check') )
 ORDER BY cl.relname, con.conname;
...
-- 5e. Negative/zero-guard violations — K3 nonneg core quantities. PASS = zero.
...
-- 5g. K4 status vocabularies — rows outside the approved lists. PASS = zero.
--     customer_subscriptions uses the RC2-WIDENED 9-value Stripe list (H4).
...
-- 5h. NOT NULL tightenings (001 C2; 002 K7/K8). PASS = rows, all 'NO'.
...
-- ============================================================================
-- SECTION 6 — RLS (004) — GENERATIVE-STRUCTURAL GAP CHECKS
-- ============================================================================

-- 6a. THE DANGEROUS STATE: RLS enabled but zero policies (locked to all
-- non-owner roles). After 004 this must be empty.
SELECT t.tablename
  FROM pg_tables t
...
-- 6b. Tenant tables WITHOUT RLS enabled. PASS = empty.
SELECT c.table_name
  FROM information_schema.columns c
...
-- 6c. Tenant policy GAP check — every organization_id base table (except the
-- two hand-handled: organizations, organization_members) must expose the four
-- `_tenant_*` policies. PASS = empty.
SELECT c.table_name
  FROM information_schema.columns c
 ...
 WHERE c.table_schema = 'public' AND c.column_name = 'organization_id'
   ...
   AND ( NOT EXISTS (SELECT 1 FROM pg_policies p WHERE p.schemaname='public' AND p.tablename=c.table_name AND p.policyname = c.table_name || '_tenant_select')
      OR NOT EXISTS (SELECT 1 FROM pg_policies p WHERE p.schemaname='public' AND p.tablename=c.table_name AND p.policyname = c.table_name || '_tenant_insert')
      OR NOT EXISTS (SELECT 1 FROM pg_policies p WHERE p.schemaname='public' AND p.tablename=c.table_name AND p.policyname = c.table_name || '_tenant_update')
      OR NOT EXISTS (SELECT 1 FROM pg_policies p WHERE p.schemaname='public' AND p.tablename=c.table_name AND p.policyname = c.table_name || '_tenant_delete') )
 ORDER BY c.table_name;

-- 6d. Policy inventory by table (informational — inspect for the hand-written
-- sets: organizations(2), users(2), organization_members(4), consultant_profiles(1),
-- consultant_firm_members(2), consultant_clients(4), + *_authenticated_read per
-- reference table, + *_tenant_* per organization_id table above).
...
-- 6e. RLS helper functions present with correct hardening. PASS = 4 rows,
-- all security definer. (Created in 004.)
SELECT p.proname, p.prosecdef AS security_definer, p.proconfig AS settings
  FROM pg_proc p
  JOIN pg_namespace n ON n.oid = p.pronamespace
 WHERE n.nspname = 'public' AND p.proname IN
       ('is_org_member','is_org_active','is_org_admin_or_owner','is_org_consultant')
 ORDER BY p.proname;

-- ============================================================================
-- SECTION 7 — TRIGGERS (006) + FUNCTIONS (005/004)
-- ============================================================================
...
-- 7b. Trigger inventory. PASS = rows named trg_set_updated_at_<table> (fewer
-- where a candidate table lacked updated_at — compare with 006 NOTICE output).
...
-- 7c. Excluded append-only logs must NOT have the maintenance trigger. PASS = empty.
...
-- 7d. Functions from 005/004 present. PASS = 6 rows, with set_updated_at marked
-- SECURITY INVOKER (prosecdef = false) and anonymise_user + the 4 RLS helpers
-- SECURITY DEFINER (prosecdef = true).
SELECT p.proname, p.prosecdef AS security_definer
  FROM pg_proc p
  JOIN pg_namespace n ON n.oid = p.pronamespace
 WHERE n.nspname = 'public'
   AND p.proname IN ('set_updated_at','anonymise_user',
                     'is_org_member','is_org_active',
                     'is_org_admin_or_owner','is_org_consultant')
 ORDER BY p.proname;

-- 7e. Views inventory. This migration adds NO views — confirm the catalogue
-- shows none created by RC2 (any present are pre-existing; record for Gate 1).
SELECT table_name AS view_name FROM information_schema.views
 WHERE table_schema = 'public' ORDER BY table_name;

-- ============================================================================
-- SECTION 8 — IRELAND-BETA READINESS + DATA-PRESERVATION NOTES
-- ============================================================================
...
-- live `region` column gone (Section 1a already proved `region` absent).
SELECT c.column_name, c.is_nullable
  FROM information_schema.columns c
 WHERE c.table_schema = 'public' AND c.table_name = 'emission_factors'
...
-- PASS = all zero (unit/scope may legitimately be NULL pending factor audit).
...
-- RC2 is additive plus renames plus NO row deletions except the deliberate
-- drop of redundant legacy columns (001 R2) on an empty table. Row counts must
-- be identical before/after, save rows touched by the staging audit (Gate 2).
--
...
-- PASS = every NOTICE reads OK; any FAIL line names the section to drill into.
-- ============================================================================
DO $$
DECLARE n bigint;
BEGIN
    ...
    SELECT count(*) INTO n FROM pg_indexes i
     WHERE i.schemaname='public' AND i.indexname IN (
       ...
       'dpq_customer_document_id_idx','dpq_emission_factor_used_idx',
       'password_reset_tokens_user_id_idx');
    RAISE NOTICE '% of 21 expected 003 indexes present', n;
...
-- ============================================================================
-- End of 007_rc2_verification.sql — read-only. Any non-empty FAIL set blocks
-- RC2 sign-off pending resolution (see 008_RC2_RELEASE_NOTES.md for the
-- per-file remediation path).
-- ============================================================================
